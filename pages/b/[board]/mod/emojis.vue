  ...
  <NuxtLayout name="settings">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Board Emojis</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Manage custom emojis for /{{ boardName }}
          </p>
        </div>
        <button
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4">
        ...
        <div class="text-sm text-gray-500 dark:text-gray-400">Board Emojis</div>
      </div>
      <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4">
      ...
      <div class="flex flex-col sm:flex-row gap-4">
        <div class="flex-1">
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Search board emojis..."
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700">
      <div v-if="loading" class="p-8 text-center">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Loading board emojis...</p>
      </div>
      ...
      <div v-else-if="filteredEmojis.length === 0" class="p-8 text-center">
        ...
        <p class="text-gray-500 dark:text-gray-400">No board emojis found</p>
        <p class="text-sm text-gray-400 mt-2">Create your first custom emoji for this board!</p>
      </div>
      ...
      >
        <div class="p-6">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
            {{ showCreateDialog ? 'Create Board Emoji' : 'Edit Board Emoji' }}
          </h2>
          ...
          <form @submit.prevent="saveEmoji" class="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                placeholder="board_special"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              />
              <p class="text-xs text-gray-500 mt-1">Used as :shortcode: in this board</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                placeholder="Board specific emoji"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              />
            </div>
...
const stats = computed(() => {
  if (!emojis.value || emojis.value.length === 0) {
    return { total: 0, active: 0, totalUsage: 0 };
  }
  return {
...
const getBoardInfo = async () => {
  try {
    ...
    const { data, error } = await useDirectGraphQLRequest(query, { name: boardName });

    if (error.value) {
      throw new Error(error.value.message || 'Failed to get board info');
    }

    if (data.value?.board) {
      boardId.value = data.value.board.id;
    }
  } catch (error) {
...
const loadEmojis = async () => {
  ...
  try {
    ...
    const { data, error } = await useDirectGraphQLRequest(query, {
      input: {
    ...
    if (error.value) {
      throw new Error(error.value.message || 'Failed to load board emojis');
    }

    if (data.value?.listEmojis) {
      emojis.value = data.value.listEmojis;
    }
  } catch (error) {
    console.error('Failed to load board emojis:', error);
    toast.addNotification({
      header: 'Error',
      message: 'Failed to load board emojis',
      type: 'error'
    });
  } finally {
    loading.value = false;
...
const saveEmoji = async () => {
  ...
  try {
    if (showCreateDialog.value) {
      ...
      // Create new board emoji using GraphQL multipart
      const mutation = `
      ...
      if (error.value) {
        throw new Error(error.value.message || 'Failed to create board emoji');
      }

      toast.addNotification({
        header: 'Success',
        message: 'Board emoji created successfully',
        type: 'success'
      });
    } else {
      ...
      if (emojiForm.value.imageFile) {
        ...
        if (error.value) {
          throw new Error(error.value.message || 'Failed to update board emoji');
        }
      } else {
      ...
      toast.addNotification({
        header: 'Success',
        message: 'Board emoji updated successfully',
        type: 'success'
      });
    }
    ...
    console.error('Failed to save board emoji:', error);
    toast.addNotification({
      header: 'Error',
      message: 'Failed to save board emoji',
      type: 'error'
    });
  } finally {
    saving.value = false;
...
const toggleEmojiStatus = async (emoji: Emoji) => {
  try {
    ...
    toast.addNotification({
      header: 'Success',
      message: `Board emoji ${emoji.isActive ? 'enabled' : 'disabled'}`,
      type: 'success'
    });
  } catch (error) {
    console.error('Failed to toggle board emoji status:', error);
    toast.addNotification({
      header: 'Error',
      message: 'Failed to update board emoji status',
      type: 'error'
    });
  }
};
...
const deleteEmoji = async (emoji: Emoji) => {
  if (!confirm(`Are you sure you want to delete the board emoji ":${emoji.shortcode}:"?`)) {
    return;
  }
  ...
  try {
    ...
    if (error.value) {
      throw new Error(error.value.message || 'Failed to delete board emoji');
    }
    ...
    toast.addNotification({
      header: 'Success',
      message: 'Board emoji deleted successfully',
      type: 'success'
    });
  } catch (error) {
    console.error('Failed to delete board emoji:', error);
    toast.addNotification({
      header: 'Error',
      message: 'Failed to delete board emoji',
      type: 'error'
    });
  }
};
