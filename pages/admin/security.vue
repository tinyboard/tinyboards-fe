	...
	<NuxtLayout name="admin">
		<div class="flex flex-col">
			...
			<div class="p-4">
				...
				<p class="mt-1 text-sm text-gray-600">Adjust the level of gatekeeping. For configuring what can or cannot be posted, go to <NuxtLink to="/admin/content">Content Settings</NuxtLink>.</p>
			</div>
			<!-- Form -->
			<form @submit.prevent="onSubmit" @submit="submitSettings()" class="sm:border sm:rounded-md overflow-y-auto">
				<div class="flex flex-col space-y-6 divide-y bg-white p-4">
					...
					<div class="md:grid md:grid-cols-3 md:gap-6 pt-4 md:pt-6">
						...
						<div class="mt-4 md:col-span-2 md:mt-0">
							<div class="flex items-center text-sm">
								<InputsSwitch id="no-registrations" :isEnabled="settings.disable_registrations" @enabled="settings.disable_registrations = !settings.disable_registrations"/>
								<label for="no-registrations" class="ml-2 font-medium text-gray-900 dark:text-gray-300">Disable new account registration</label>
							</div>
							<p class="mt-2 text-sm text-gray-500">
