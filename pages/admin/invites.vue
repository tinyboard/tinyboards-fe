		...
		<div class="flex flex-col bg-white overflow-hidden shadow-inner-xs sm:border sm:rounded-md">
			...
			<div class="flex p-4 border-b">
				<div>
					...
					<p class="mt-1 text-sm text-gray-600">Here is a list of active invites to your tinyboard. Click the trash button to revoke.</p>
				</div>
				<button v-if="site.site_mode === 'InviteMode'" @click="createInvite" class="ml-auto flex items-center button green">
					<svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 mr-2" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
			...
			<ul v-if="invites?.invites.length" class="flex flex-col">
				<li v-for="invite in invites.invites" :key="invite.id" class="relative group grid grid-cols-6 p-4 odd:bg-gray-50 hover:bg-gray-100 border-b last:border-0 shadow-inner-white">
