	    ...
	    <section class="container mx-auto max-w-7xl max-w-7xl grid grid-cols-12 pt-10">
			...
			<div class="col-span-full flex gap-6 py-8 sm:px-6 lg:px-8 xl:px-0 my-2.5 sm:my-0">
				...
				<div class="w-full bg-gray-200/50 shadow-inner-xs sm:rounded-lg">
					...
					<!-- Details -->
			    	<div id="details" class="relative flex flex-wrap md:flex-nowrap items-center w-full p-4 sm:p-6 bg-cover bg-center rounded-b-lg" :style="{ backgroundImage: `url(https://i.imgur.com/HRYqKAf.jpg)` }">
			    		<!-- Avatar -->
				    	<img
						loading="lazy"
			...
			<div class="col-span-full flex gap-6 py-8 sm:px-6 lg:px-8 xl:px-0 my-2.5 sm:my-0">
             	<div class="relative w-full overflow-x-auto rounded-lg">
				    <table class="table-fixed w-full text-sm text-left text-gray-500 dark:text-gray-400">
				        <thead class="text-sm text-gray-700 w-full p-4 lg:p-6 bg-gray-200/50 shadow-inner-xs rounded-lg">
				            <tr>
				                <th scope="col" class="w-3/5 py-4 px-4 sm:px-6 font-medium">
				                    Recent posts
				                </th>
				                <th scope="col" class="text-center py-4 px-4 sm:px-6 font-normal">
				                    Points
				                </th>
				                <th scope="col" class="text-center py-4 px-4 sm:px-6 font-normal">
				                    Replies
				                </th>
				                <th scope="col" class="hidden lg:table-cell text-center py-4 px-6 font-normal">
				                    Views
				                </th>
				                <th scope="col" class="hidden xl:table-cell py-4 px-6 font-normal">
				                    Last reply
				                </th>
				            </tr>
				        </thead>
				        <tbody>
				            <tr v-for="item in items" :key="item.id" class="bg-white border-b last:border-0 dark:bg-gray-800 dark:border-gray-700">
				                <th scope="row" class="flex items-center space-x-2 py-4 px-4 sm:px-6 font-medium text-gray-900 dark:text-white sm:truncate">
				                	<NuxtLink class="text-gray-900 visited:text-gray-400 hover:text-primary sm:overflow-hidden sm:text-ellipsis" :to="`/post/${item.id}`">
										{{ item.title }}
									</NuxtLink>
				                </th>
				                <td class="text-center py-4 px-4 sm:px-6">
				                	{{ item.points }}
				                </td>
				                <td class="text-center py-4 px-4 sm:px-6">
				                	{{ item.replies }}
				                </td>
				                <td class="hidden lg:table-cell text-center py-4 px-6">
				                    {{ item.views }}
				                </td>
