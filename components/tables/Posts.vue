<template>
  <div class="relative w-full sm:rounded-md sm:border overflow-x-auto">
    ...
    >
      <thead
        class="text-base text-gray-700 leading-5 w-full px-4 py-3 lg:p-6 bg-gray-200/50 border-b shadow-inner-white"
      >
        <tr>
      ...
      <tbody v-if="isLoading">
        <tr
          v-for="i in 7"
          :key="i"
          class="bg-white odd:bg-gray-50 border-b last:border-0 dark:bg-gray-800 dark:border-gray-700"
        >
          <th
            scope="row"
            class="flex items-center px-4 py-3 text-gray-900 dark:text-white animate-pulse"
          >
            <div class="flex-shrink-0 w-12 h-12 bg-gray-100"></div>
            <div
              class="ml-3 bg-gray-100 h-3"
              :class="i % 2 === 0 ? 'w-1/3' : 'w-1/2'"
            ></div>
          </th>
          <td
            v-for="j in 3"
            :key="j"
            class="hidden xl:table-cell py-4 px-4 animate-pulse"
          >
            <div
              class="bg-gray-100 h-3"
              :class="i % 2 === 0 ? 'w-1/3' : 'w-1/2'"
            ></div>
          </td>
        </tr>
      ...
      <tbody v-else-if="posts.length">
        <tr
          ...
          :class="
            isAdmin && item.post.is_removed
              ? 'bg-red-100 border-red-200 hover:bg-red-50'
              : 'bg-white odd:bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700'
          "
        >
          <th
            v-if="item.creator"
          ...
          <td class="hidden lg:table-cell py-4 px-4">
            <div
              :title="`${item.counts.upvotes} ${
                item.counts.upvotes === 1 ? 'upvote' : 'upvotes'
              } | ${item.counts.downvotes} ${
                item.counts.downvotes === 1 ? 'downvote' : 'downvotes'
              }`"
          ...
          <td v-if="item.counts" class="hidden lg:table-cell py-4 px-4">
            <div
              :title="`${item.counts.comments} ${
                item.counts.comments === 1 ? 'comment' : 'comments'
              } on this post`"
              class="flex items-center"
            >
              <svg
          ...
          <td v-if="item.counts" class="hidden xl:table-cell py-4 px-4">
            <div
              :title="`Last reply ${item.counts.newest_comment_time}`"
              class="flex items-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
      ...
      <tbody v-else class="empty_state">
        <tr v-for="i in 7" :key="i" class="bg-white dark:bg-gray-800">
          ...
          <td
            v-for="i in 3"
            :key="i"
            class="hidden xl:table-cell py-4 px-4"
          ></td>
        </tr>
      </tbody>
    </table>
