<template>
  <div class="toast relative flex justify-between rounded-md shadow-lg border transition-all duration-200" :class="notification.type === 'success' ? 'bg-green-300 border-green-400' : 'bg-red-200 border-red-300'">
    <!-- Content -->
    <div class="toast-content flex items-center pr-3">
      <!-- Icon -->
      <div class="p-4">
    ...
    <!-- Close Button -->
    <button class="toast-close absolute top-1 right-1 w-8 h-8 flex items-center justify-center rounded bg-transparent" :class="notification.type === 'success' ? 'text-green-700 hover:text-green-900 hover:bg-green-400' : 'text-red-700 hover:text-red-900 hover:bg-red-300'" aria-label="Close toast" @click="toast.removeNotification(notification)">
      <span>&#x2715</span>
    </button>
  </div>
</template>
