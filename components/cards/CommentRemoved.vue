      ...
      <div class="flex-grow" :class="{ 'flex comments-center': isCollapsed }">
        ...
        <ul class="relative flex flex-grow flex-wrap comments-center space-x-4" v-show="!isCollapsed">
          <li class="hidden sm:inline sm:list-comment">
            <NuxtLink
              :to="parentPost?.urlPath ? `${parentPost.urlPath}/${comment.id}#comment-text-${comment.id}` : `${site.enableBoards && comment.board ? '/b/' + comment.board.name + '/p' : '/p'}/${comment.post?.id}/${parentPost?.slug || parentPost?.titleChunk || '-'}/${comment.id}#comment-text-${comment.id}`"
          ...
          <li class="hidden sm:inline sm:list-comment">
            <NuxtLink
              :to="parentPost?.urlPath ? `${parentPost.urlPath}/${comment.id}?context=3#comment-text-${comment.id}` : `${site.enableBoards && comment.board ? '/b/' + comment.board.name + '/p' : '/p'}/${comment.post?.id}/${parentPost?.slug || parentPost?.titleChunk || '-'}/${comment.id}?context=3#comment-text-${comment.id}`"
              class="text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 font-medium">
              Context
            </NuxtLink>
          </li>
        ...
        <NuxtLink v-if="comment.replyCount && level > limit" v-show="!isCollapsed"
          :to="parentPost?.urlPath ? `${parentPost.urlPath}/${comment.id}` : `${site.enableBoards && comment.board ? '/b/' + comment.board.name + '/p' : '/p'}/${comment.post?.id}/${parentPost?.slug || parentPost?.titleChunk || '-'}/${comment.id}`"
...
const parentPost: PostFragment | undefined = comment.post ?? postStore.getPost(comment.post?.id);
</script>
