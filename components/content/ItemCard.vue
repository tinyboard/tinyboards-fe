<template>
	<div class="relative bg-white border-t border-b border-gray-100 sm:shadow-xs dark:border-transparent sm:border-0 sm:rounded-md dark:bg-white">
		<div class="flex flex-col flex-grow">
			<!-- Post Details -->
			<div class="flex flex-shrink-0 items-center justify-between p-2.5 border-b">
				<div class="flex items-center w-full overflow-x-auto">
					<!-- Author Avatar -->
					<NuxtLink :to="'/'+item.author.username" class="mr-2">
						<img
						:src="item.author.avatarUrl"
						alt="avatar"
						class="w-9 h-9 md:w-7 md:h-7 object-cover rounded-sm bg-gray-100"
						/>
					</NuxtLink>
					<div class="flex flex-col sm:flex-row sm:items-center sm:space-x-2 leading-4">
						<!-- Author Username -->
						<NuxtLink :to="'/'+item.author.username" class="block font-medium text-base text-gray-900">
				...
				<button class="flex items-center justify-center md:hidden -mr-1 p-1 text-gray-600">
			...
			<div class="flex justify-between p-2.5">
				<div>
					...
					<h5 class="text-md font-medium">
						<NuxtLink class="text-gray-900 visited:text-gray-400 hover:text-primary" :to="`/post/${item.id}`">
							{{ item.title }}
						</NuxtLink>
					</h5>
					<!-- Link Post Domain -->
					<a :href="item.url" v-if="item.url && item.type !== 'image'" target="_blank" class="block text-sm text-gray-500 hover:underline">
						{{ item.domain }}
				...
				<div class="flex-shrink-0 pl-3">
					<!-- Thumbnail -->
					<div v-if="item.thumbUrl" class="relative">
						<img
						v-if="item.type === 'image'"
						:src="item.thumbUrl"
						alt="Post image"
						class="w-14 md:w-20 h-14 object-cover rounded-sm bg-gray-200"
					...
					<!-- Fallback Icon -->
					<div v-else class="hidden md:flex items-center justify-center w-20 h-14 rounded-sm bg-gray-100">
