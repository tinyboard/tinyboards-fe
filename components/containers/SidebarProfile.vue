<template>
  ...
  <div v-if="isAdmin && !isSelf" :class="{ 'bg-white rounded border border-gray-200 p-4': (imageStore.background || u.profile_background) }">
    ...
    <ul class="flex flex-col space-y-2 mt-4">
      ...
      <li v-if="!u.is_admin">
        <button class="flex items-center button w-full" :class="[u.is_banned ? 'green' : 'red']" @click="confirmBan">
          ...
          <span>{{ u.is_banned ? 'Unban' : 'Ban' }} @{{ u.name }}</span>
        </button>
      </li>
    </ul>
  </div>
  <div v-if="site.enableBoards && moderates.length" :class="{ 'bg-white rounded border border-gray-200 p-4': (imageStore.background || u.profile_background) }">
    ...
    <ul class="flex flex-col mt-4 space-y-2 divide-y divide-gray-200/50">
      ...
      <NuxtLink :to="`/+${m.board.name}`" class="flex space-x-2">
        ...
        :src="m.board.icon ?? 'https://placekitten.com/36/36'"
        />
        <div class="flex flex-col justify-center leading-normal">
          <div class="flex">
            <strong class="text-sm">{{ m.board.title ?? m.board.name }}</strong>
          </div>
