<template>
    ...
    <router-link :to="`/b/${board.name}/submit`" custom v-slot="{ navigate }">
		<button class="button primary w-full text-left" @click="navigate" @keypress.enter="navigate" role="link">
      ...
      <div>
        ...
        <ul class="space-y-1">
            ...
            <li>
                <button class="w-full bg-gray-300 bg-opacity-0 hover:bg-opacity-40 p-2 rounded text-left text-red-500">
      ...
      <!--<div v-if="users">
        <div class="flex items-end mb-3 pb-1 border-b">
          <h2 class="font-bold leading-5 text-base">Moderators</h2>
          <NuxtLink
            to="/members"
