<template>
 ...
 >
 <MenuItems class="absolute w-48 max-h-72 mt-1 bg-white dark:bg-gray-900 rounded-md shadow-xl border dark:border-gray-800 focus:outline-none" :class="isLeft ? 'right-0 origin-top-right' : 'left-0 origin-top-left'">
  <div class="py-2">
    <!-- Menu Item -->
    <MenuItem v-slot="{ active, close }" v-for="(item, index) in sorts" :key="index">
      <NuxtLink :to="item.href" :class="[index === selectedSort ? 'font-medium text-secondary' : 'font-normal text-gray-700 dark:text-gray-300',{ 'bg-gray-100 dark:bg-gray-800':active },'block text-sm px-4 py-2 truncate']" @click="sort = item.key; close()">
        {{ item.name }}
      </NuxtLink>
    </MenuItem>
  </div>
