<template>
  <Menu as="div" class="relative z-20">
    ...
    <transition leave-active-class="transition ease-[cubic-bezier(.2,0,0,1.3)]"
      ...
      <MenuItems
        class="absolute w-56 max-h-72 mt-1 bg-white divide-y divide-gray-100 rounded-md shadow-xl border focus:outline-none right-0 origin-top-right">
        <!-- Board Actions -->
        <div class="py-2 text-sm">
          <MenuItem disabled v-slot="{ active, close }">
            <button @click="close()" class="group flex items-center w-full px-4 py-1.5"
              ...
              <svg  xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-2"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 14h14l-4.5 -4.5l4.5 -4.5h-14v16" /></svg>
          ...
          <MenuItem disabled v-slot="{ active, close }">
            <button @click="close()" class="group flex items-center w-full px-4 py-1.5"
              :class="active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'">
              <svg  xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-2"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" /><path d="M5.7 5.7l12.6 12.6" /></svg>
              <span>Block +{{ board.name }}</span>
            </button>
          </MenuItem>
        </div>
        <!-- Mod Actions -->
        <div v-if="isAdmin" class="py-2 text-sm border-t">
          <!-- Manage Admin -->
          <MenuItem v-if="!isMod" disabled v-slot="{ active, close }">
            <button @click="close()" class="group flex items-center w-full px-4 py-1.5"
              :class="active ? 'bg-green-100 text-green-900' : 'text-green-700'">
          ...
          <MenuItem disabled v-slot="{ active, close }">
            <button @click="close()" class="group flex items-center w-full px-4 py-1.5"
              :class="active ? 'bg-red-100 text-red-900' : 'text-red-700'">
          ...
          <MenuItem>
            <button @click="close()" class="group flex items-center w-full px-4 py-1.5"
                :class="active ? 'bg-yellow-100 text-yellow-900' : 'text-yellow-700'">
