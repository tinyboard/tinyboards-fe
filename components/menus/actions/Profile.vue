<template>
  <Menu as="div" class="relative z-10">
    ...
    >
      <MenuItems class="absolute w-48 max-h-72 mt-1 bg-white divide-y divide-gray-100 rounded-md shadow-xl border focus:outline-none" :class="isLeft ? 'right-0 origin-top-right' : 'left-0 origin-top-left'">
        <!-- Profile Actions -->
        <div v-if="isSelf" class="py-2 text-sm">
          <MenuItem disabled v-slot="{ active, close }">
            <NuxtLink to="/settings/profile" @click="close()" class="group flex items-center w-full px-4 py-1.5" :class="active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'">
        ...
        <div v-if="!isSelf" class="py-2 text-sm">
          <MenuItem disabled v-slot="{ active, close }">
            <button @click="close()" class="group flex items-center w-full px-4 py-1.5" :class="active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-2" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                 <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
        ...
        <div v-if="isAdmin && !isSelf" class="py-2 text-sm">
          ...
          <!-- Purge Content -->
          <MenuItem :disabled="user.is_admin" v-slot="{ active, close }">
            <button @click="confirmPurge(); close()" class="group flex items-center w-full px-4 py-1.5" :class="active ? 'bg-gray-100 text-red-700' : 'text-red-500 hover:text-red-700'">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-2" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                 <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                 <path d="M19 20h-10.5l-4.21 -4.3a1 1 0 0 1 0 -1.41l10 -10a1 1 0 0 1 1.41 0l5 5a1 1 0 0 1 0 1.41l-9.2 9.3"></path>
                 <path d="M18 13.3l-6.3 -6.3"></path>
              </svg>
              <span>Purge {{ user.username }}</span>
            </button>
          </MenuItem>
        </div>
      </MenuItems>
