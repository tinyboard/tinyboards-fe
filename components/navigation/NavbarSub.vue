			...
			<ul class="flex items-center overflow-x-auto scrollbar-hidden p-[1px] m-[-1px]">
				<li class="text-sm leading-6 flex-shrink-0" v-for="link in links" :key="link.name">
					<NuxtLink :to="link.href" custom v-slot="{ href, navigate }">
						<a :href="href" :target="link.target" @click="navigate" :class="[href === route.fullPath && !disableActiveClass ? 'text-primary shadow-[0_1px_0_rgba(var(--color-primary))]' : 'text-gray-500 hover:text-primary', 'px-4 py-2 block']">
							{{ link.name }}
						</a>
					</NuxtLink>
				</li>
